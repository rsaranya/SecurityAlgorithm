...
public class GlobalObjects {
	...
	public static enum BYTE_ROUND {
		...
		/**
		 * This function gets the round number based on the input.
		 *
		 * @param pbyteRound
		 * @return the corresponding number of rounds
		 */
		...
		/**
		 * This function returns the enum based on the length of the key.
		 *
		 * @param pintBytes
		 *            contains the length of the string
		 * @return the appropriate enum based on
		 */
	...
	public static char lstrDelimiter = '&';
	...
	static String[][] GALOIS_MATRIX = { { "02", "03", "01", "01" }, { "01", "02", "03", "01" },
			{ "01", "01", "02", "03" }, { "03", "01", "01", "02" } };

	// The fixed matrix used for column mixing
	static String[][] INV_GALOIS_MATRIX = { { "0E", "0B", "0D", "09" }, { "09", "0E", "0B", "0D" },
			{ "0D", "09", "0E", "0B" }, { "0B", "0D", "09", "0E" } };

	/**
	 * Returns a value from the Round-Key Table using the specified row and
	 * column values
	 *
	 * @param pintRow
	 *            : Contains the row number from which the value is to be
	 *            fetched from the Round key table.
	 * @param pintCol
	 *            : Contains the column number from which the value is to be
	 *            fetched from the Round key table.
	...
	public static String shiftBit(String pstrHexValue) {
		...
		case DECRYPT:
			shiftedBinary = Integer.toBinaryString(intvalueIn >> 1);
			shiftedBinary = (shiftedBinary.length() > 8
					? shiftedBinary.substring(shiftedBinary.length() - 8, shiftedBinary.length()) : shiftedBinary);
			break;
		case NONE:
		default:
			shiftedBinary = "";
	...
	public static String computeXOR(String pstrFirstHex, String pstrSecondHex) {
		try {
			...
			return lstrXorResult.length() == 1 ? ("0" + lstrXorResult) : lstrXorResult;
		} catch (Exception ex) {
				...
				if (colCount % gintArrayColSize != 0) {
					...
					if (genumNoOfRounds == BYTE_ROUND.BYTE_64 && colCount % 4 == 0) {
						for (int row = 0; row < gintArrayRowSize; row++) {
							larWMatrix[row][colCount] = aesSBox(larWMatrix[row][colCount - 1].split(""));

							larWMatrix[row][colCount] = computeXOR(larWMatrix[row][colCount - gintArrayColSize],
					...
					for (int col = 0; col < gintArrayRowSize; col++) {
						...
						String[] larSplitHexValue = larWMatrix[wRowValue][colCount - 1].split("");
					...
					String lstrRconValue = aesRcon((int) Math.floor(colCount / (gintArrayColSize * 16)),
					...
					for (int rowCount = 0; rowCount < gintArrayRowSize; rowCount++) {
						larWMatrix[rowCount][colCount] = computeXOR(larWMatrix[rowCount][colCount - gintArrayColSize],
								larWnew[0][rowCount]);
					}
	...
	public static void generateKeyMatrix(String pstrInputKey) {
		...
		try {
			int col = 0;
			for (int colCounter = 0; colCounter < (pstrInputKey.length() - 1); colCounter += 8, col++) {
				int row = 0;
				for (int rowCounter = colCounter; rowCounter < (colCounter + 8); rowCounter += 2, row++) {
					larInputKey[row][col] = pstrInputKey.substring(rowCounter, (rowCounter + 2));
				}
			}
		} catch (Exception ex) {
	...
	public static void printRoundKeys() {
		try {
			int counter = GlobalObjects.BYTE_ROUND.getValue(GlobalObjects.genumNoOfRounds) + 1;
			int col = 0;
			...
			for (int i = 0; i < inStateHex.length; i++, row++) {
				for (int j = 0; j < inStateHex[0].length; j++) {
					...
					case DECRYPT:
						...
						int outpuTCol = (lintDiff < 0 ? inStateHex.length + lintDiff : lintDiff);
		...
		try {
			...
			for (int colCounter = 0; colCounter < (pstrInputText.length()
				...
				for (int rowCounter = colCounter; rowCounter < (colCounter
					...
					larInputData[row][col] = pstrInputText.substring(rowCounter, (rowCounter + 2));
				}
			}
		} catch (Exception ex) {
