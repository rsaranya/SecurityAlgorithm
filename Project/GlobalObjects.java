...
public class GlobalObjects {
  ...
  public static enum BYTE_ROUND {
     ...
     *
     * @param pintBytes contains the length of the string
    ...
    switch (genumAlgoMode) {
      case ENCRYPT:
        ...
        shiftedBinary = (shiftedBinary.length() > 8 ? 
                shiftedBinary.substring(1) : shiftedBinary);
        break;
      case DECRYPT:
        shiftedBinary = Integer.toBinaryString(intvalueIn >> 1);
        shiftedBinary = (shiftedBinary.length() > 8
                ? shiftedBinary.substring(shiftedBinary.length() - 8,
                        shiftedBinary.length()) : shiftedBinary);
        break;
      case NONE:
      default:
        shiftedBinary = "";
    ...
    try {
      ...
      for (; colCount < larWMatrix[0].length; colCount++) { // larWMatrix[0].length
        ...
        if (colCount % gintArrayColSize != 0) {
          ...
          String lstrRconValue = aesRcon((int) Math.floor(colCount /
                  (gintArrayColSize * 16)),
      ...
      System.out.println("Exception in generateWMatrix is : " 
              + ex.getMessage());
    }
  }
    ...
    try {
      ...
      for (int colCounter = 0; colCounter < (pstrInputKey.length() - 1);
        ...
        for (int rowCounter = colCounter; rowCounter < (colCounter + 8);
                rowCounter += 2, row++) {
          larInputKey[row][col] = pstrInputKey.
                  substring(rowCounter, (rowCounter + 2));
        }
      }
    } catch (Exception ex) {
      System.out.println("Exception in generateKeyMatrix is : " +
              ex.getMessage());
    }
  }
  ...
  public static void printRoundKeys() {
    try {
      int counter = GlobalObjects.BYTE_ROUND.getValue(
              GlobalObjects.genumNoOfRounds) + 1;
      int col = 0;
        ...
        for (int j = 0; j < inStateHex[0].length; j++) {
          switch (genumAlgoMode) {
            ...
            case DECRYPT:
              ...
              int outpuTCol = (lintDiff < 0 ? inStateHex.length +
                      lintDiff : lintDiff);
          ...
          if (null != lstrGalois_Matrix) {
            switch (lstrGalois_Matrix) {
              ...
              case "01":
                ...
                break;
              case "0E":
                int inputInHex = Integer.parseInt(
                        inStateHex[stateRow][stateCol], 16);
                sum = computeXOR(sum, Integer.toHexString(
                        calculateForE(inputInHex)));

                break;
              case "0B":
                inputInHex = Integer.parseInt(
                        inStateHex[stateRow][stateCol], 16);
                sum = computeXOR(sum, Integer.toHexString(
                        calculateForB(inputInHex)));
  ...
  public static void generateDataMatrix(String pstrInputText) {
    ...
    try {
      ...
      for (int colCounter = 0; colCounter < (pstrInputText.length()
              - 1); colCounter += (GlobalObjects.gintArrayRowSize * 2),
              col++) {
        int row = 0;
        for (int rowCounter = colCounter; rowCounter < (colCounter
                + (GlobalObjects.gintArrayRowSize * 2)); 
                rowCounter += 2, row++) {
      ...
      System.out.println("Exception in generateDataMatrix is : " + 
              ex.getMessage());
    }
  }
   ...
   *
   * @param InputHex : Input from State XOR
   ...
   *
   * @param pintInputInHex : Input from State XOR
   * @return : Calculated value to reverse mix columns
   */
