 ...
 * @Program: Utilizes the AES Key generation class to generate round keys Create
 * a cipher text from a given plain text and Decrypt the cipher text to retrieve
 * the original Text
 */
import java.util.*;
...
public class Driver {
   ...
   *
   * @param args : The command line argument containing the file name.
   */
    ...
    try {
      ...
      if ((inputKey.matches("[0-9A-F]+$"))
        ...
        GlobalObjects.genumNoOfRounds
                = GlobalObjects.BYTE_ROUND.getEnum(lintInputKeySize);
        ...
        GlobalObjects.gintWArrayColSize = 4
                * (GlobalObjects.BYTE_ROUND.getValue(
   ...
   * 32-bytes. Encrypts the Input Data using the Keys And Decrypts the Encrypted
   * Text.
   *
   * @param pstrInputText: Contains the Input Text entered by the user.
   * @param pstrInputKey: Contains the Input Key entered by the user.
   */
  private static void checkForPaddingAndEncrypt(String pstrInputText,
          String pstrInputKey) {
    ...
    for (int noOfBlocks = 0; noOfBlocks < loopCount; noOfBlocks++) {
      int minLength = Math.min(GlobalObjects.gintInputBlockSize,
              (pstrInputText.length() - blockSent
              * GlobalObjects.gintInputBlockSize));
      strBlock[noOfBlocks] = pstrInputText.substring(startIndex,
   ...
   *
   * @param pstrInputText : Contains the Input Text to be checked and appended
   * with the delimiter.
  ...
  private static String appendAnExtraDelimiter(String pstrInputText) {
    ...
    String lstrHexOfDelimiter = Integer.toHexString(
    ...
    if (pstrInputText.substring(pstrInputText.length() - 2,
            pstrInputText.length()).equals(lstrHexOfDelimiter)) {
