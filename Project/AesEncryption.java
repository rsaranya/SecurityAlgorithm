 ...
 * @Program Generates 10, 12 or 14 round keys for AES algorithm based on an input key size.
 * Uses the input key and input plain text to generate cipher text
 */
...
public class AesEncryption {
	...
	public final void aesRoundKeys(String pstrInputText, String pstrInputKey) {
		try {
			...
			System.out.println("Exception in aesRoundKeys() in " + "AesEncryption.java " + ex.getMessage());
		}
	}
	...
	protected String[][] computeDataForEachRound(String[][] larRoundData, String[][] larRoundKey, int pintRoundCount) {
		...
		try {
			...
			System.out.println("Error in computeDataForEachRound() " + "in AesEncryption.java: " + ex.getMessage());
		}
	...
	private String EncryptUsingAes() {
		try {
			...
			for (int increment = 0; increment < GlobalObjects.gintWArrayColSize && roundCount <= noOfRounds;) {
				// Copy the 4x4 Round Key matrix from the global 4x44 | 4x48 |
				// 4x56 matrix
				// into a local Matrix.
				for (int col = 0; col < 4 && increment < GlobalObjects.gintWArrayColSize; col++, increment++) {
			...
			System.out.println("Exception in EncryptUsingAes() in " + "AesEncryption.java:" + ex.getMessage());
		}
		return "";
	}
	 ...
	 *
	 * @param pstrPlainText
	 *            contains the text that needs to be padded
	...
	protected String addPadding(String pstrPlainText) {
		try {
			...
			if ((pstrPlainText.length() % GlobalObjects.gintInputBlockSize) != 0) {
				...
				lstrPaddedText.append(Integer.toHexString((int) (GlobalObjects.lstrDelimiter)));
				int noCharsToPad;
				if (lstrPaddedText.length() % GlobalObjects.gintInputBlockSize != 0) {
					noCharsToPad = (((lstrPaddedText.length() / GlobalObjects.gintInputBlockSize) + 1)
							* GlobalObjects.gintInputBlockSize) - lstrPaddedText.length();
				} else {
					noCharsToPad = (lstrPaddedText.length() / GlobalObjects.gintInputBlockSize)
							* GlobalObjects.gintInputBlockSize - lstrPaddedText.length();
				}
